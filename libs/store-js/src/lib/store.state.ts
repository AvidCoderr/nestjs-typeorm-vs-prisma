...
import { deepFreeze, getUniqueString, isFunction } from "@yetoman/util";
...
export class StoreState<T = StoreStateType> {
  ...
  claimSlice(sliceName: string, logger?: StoreLogger): string {
    this.registry.forEach((entity) => {
      if (entity.sliceName === sliceName) {
        throw new Error(
          `claimSlice: Slice ${sliceName} already claimed (${entity.sliceName})`
        );
      }
    });
  ...
  releaseSlice(claimId: string) {
    ...
    if (!entity) {
      throw new Error(
        `releaseSlice: No slice found with claim ID: (${claimId})`
      );
    }
  ...
  setState<K = any>(claimId: string, updater: K, action?: string): K {
    ...
    if (!entry) {
      throw new Error(
        `setState: No slice registration with private key: (${claimId})`
      );
    }
